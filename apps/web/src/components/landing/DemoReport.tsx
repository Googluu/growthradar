  ...
  return (
    <div style={{ position: "relative", width: 200, height: 200, flexShrink: 0 }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontFamily: "var(--font-syne)", fontWeight: 800, fontSize: 44, color, lineHeight: 1 }}>{score}</span>
  ...
  return (
    <div style={{ paddingBottom: 10, marginBottom: 2 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontFamily: "var(--font-dm-sans)", fontSize: 13, color: "rgba(255,255,255,0.5)" }}>{label}</span>
      ...
      <div className="demo-bar">
        <div className={`demo-bar-fill${started ? " animate" : ""}`}
  ...
  return (
    <div className="demo-check-row">
      <span style={{
        ...
        background: ok ? "rgba(93,184,72,0.18)" : "rgba(229,57,53,0.15)", fontSize: 10,
      }}>
        <span style={{ color: ok ? "#5DB848" : "#E53935" }}>{ok ? "✓" : "✕"}</span>
      </span>
      <span style={{ fontFamily: "var(--font-dm-sans)", fontSize: 13, color: ok ? "rgba(255,255,255,0.7)" : "rgba(255,255,255,0.45)" }}>
        {label}
      </span>
    </div>
  );
...
function RecCard({ rec, delay }: { rec: Recommendation; delay: number }) {
  return (
    <div className="demo-rec-card" style={{ border: `1px solid ${rec.accent}44`, animationDelay: `${delay}ms` }}>
      ...
      <h4 style={{ fontFamily: "var(--font-syne)", fontWeight: 800, fontSize: 16, color: "rgba(255,255,255,0.92)", marginBottom: 12, letterSpacing: "-0.3px", lineHeight: 1.3 }}>
        {rec.title}
      </h4>
      <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.3)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 4 }}>Problema</p>
      <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 13, color: "rgba(255,255,255,0.5)", lineHeight: 1.6, marginBottom: 12 }}>{rec.problem}</p>
      <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 11, fontWeight: 700, color: rec.accent, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 4 }}>Acción</p>
      <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 13, color: "rgba(255,255,255,0.65)", lineHeight: 1.6, marginBottom: 16 }}>{rec.action}</p>
...
// ── Helpers ────────────────────────────────────────────────────────────────────
function scoreIcon(score: number): { color: string; icon: string } {
  if (score >= 75) return { color: "#5DB848", icon: "✓" };
  if (score >= 50) return { color: "#F5C842", icon: "⚠" };
  return { color: "#E53935", icon: "✕" };
}

function scoreBadge(score: number) {
  if (score >= 75) return { bg: "rgba(93,184,72,0.15)",   color: "#5DB848", border: "1px solid rgba(93,184,72,0.3)"  };
  if (score >= 50) return { bg: "rgba(245,200,66,0.12)",  color: "#F5C842", border: "1px solid rgba(245,200,66,0.3)" };
  return             { bg: "rgba(229,57,53,0.12)",   color: "#E53935", border: "1px solid rgba(229,57,53,0.3)"  };
}

// ── Demo fallback data (dian.gov.co) ──────────────────────────────────────────
const DEMO_DATA: AuditResult = {
  url: "dian.gov.co",
  ...
  ai_summary:
    ...
    "y tráfico estimado de $6.1M USD, una optimización SEO básica podría triplicar tu visibilidad en 60 días.",
  metrics: [
  ...
  recommendations: [
    {
      badge: "QUICK WIN · ALTO IMPACTO", accent: "#5DB848",
      ...
      impact: "+15–30% CTR en búsquedas", effort: "2–4 horas",
    },
    {
      badge: "QUICK WIN · ALTO IMPACTO", accent: "#A3C94A",
      ...
      impact: "+20% relevancia semántica", effort: "1–2 horas",
    },
    {
      badge: "ESTRATÉGICO · CRÍTICO", accent: "#F5C842",
      ...
      impact: "Mejora directa Core Web Vitals", effort: "1–2 días dev",
    },
  ],
};
...
export function DemoReport({ reportData, url, onUrlChange, onScan }: Props) {
  ...
  const perfIcon = scoreIcon(data.scores.performance);
  const seoIcon  = scoreIcon(data.scores.seo);
  const perfBadge = scoreBadge(data.scores.performance);
  const seoBadge  = scoreBadge(data.scores.seo);

  return (
    <section id="demo-section" ref={ref} style={{ background: "var(--bg)", padding: "96px 24px" }}>
      <div className="max-w-275 mx-auto">
        ...
        >
          <div style={{
            ...
            <span style={{ color: "#5DB848", fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", fontFamily: "var(--font-dm-sans)" }}>
              {isDemo ? "DEMO INTERACTIVO" : "TU REPORTE"}
            </span>
          </div>
          <h2 style={{
          ...
          <p style={{ color: "var(--txt-muted)", fontSize: 16, maxWidth: 520, margin: "0 auto", fontFamily: "var(--font-dm-sans)" }}>
            {isDemo
              ? <>Análisis real de <strong style={{ color: "var(--txt)" }}>dian.gov.co</strong> — así se ve tu reporte</>
              : <>Reporte completo de <strong style={{ color: "var(--txt)" }}>{data.url}</strong></>
            }
          </p>
        </div>
        ...
        <div className="demo-row1 mb-5">
          ...
          <div style={{
            ...
            <ScoreRing score={data.scores.overall} started={inView} />
            <div style={{ display: "flex", flexDirection: "column", gap: 8, width: "100%" }}>
              {[
                { label: "PERFORMANCE", ...perfIcon },
                { label: "SEO",         ...seoIcon  },
                ...
                <div key={b.label} style={{
                  ...
                  <span style={{ fontFamily: "var(--font-dm-sans)", fontSize: 12, fontWeight: 700, color: b.color, letterSpacing: "0.07em" }}>{b.label}</span>
          ...
          <div className="demo-ai-card" style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                <div style={{ fontFamily: "var(--font-syne)", fontWeight: 800, fontSize: 17, color: "var(--txt)" }}>Análisis de Claude AI</div>
                <div style={{ fontSize: 11, color: "var(--txt-faint)", fontFamily: "var(--font-dm-sans)" }}>claude-sonnet-4-6 · 2.5k tokens</div>
              </div>
            </div>

            {/* Demo: JSX con highlights. Real: texto plano del backend */}
            {isDemo ? (
              <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 14, color: "rgba(255,255,255,0.65)", lineHeight: 1.75, flexGrow: 1, borderLeft: "1px solid rgba(255,255,255,0.08)", paddingLeft: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              <span style={{ background: "rgba(93,184,72,0.12)", border: "1px solid rgba(93,184,72,0.3)", color: "#5DB848", borderRadius: 20, padding: "5px 14px", fontSize: 12, fontWeight: 700, fontFamily: "var(--font-dm-sans)", letterSpacing: "0.06em" }}>POTENCIAL ALTO</span>
              <span style={{ color: "var(--txt-faint)", fontSize: 12 }}>·</span>
              <span style={{ background: "rgba(245,200,66,0.1)", border: "1px solid rgba(245,200,66,0.3)", color: "#F5C842", borderRadius: 20, padding: "5px 14px", fontSize: 12, fontWeight: 700, fontFamily: "var(--font-dm-sans)", letterSpacing: "0.06em" }}>Acción rápida</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
          ...
          <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 16, padding: "24px 28px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <div style={{ background: perfBadge.bg, color: perfBadge.color, border: perfBadge.border, borderRadius: 20, padding: "3px 12px", fontFamily: "var(--font-dm-sans)", fontSize: 12, fontWeight: 700 }}>
                {data.scores.performance} / 100
              </div>
            </div>
            <div>
              {data.metrics.map((m) => <MetricBar key={m.label} {...m} started={inView} />)}
            </div>
          ...
          <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 16, padding: "24px 28px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <div style={{ background: seoBadge.bg, color: seoBadge.color, border: seoBadge.border, borderRadius: 20, padding: "3px 12px", fontFamily: "var(--font-dm-sans)", fontSize: 12, fontWeight: 700 }}>
                {data.scores.seo} / 100
              </div>
            </div>
            <div style={{ marginBottom: 18 }}>
              {data.checks.map((c, i) => <CheckRow key={i} {...c} />)}
            </div>
            <div style={{ height: 1, background: "var(--border)", margin: "0 0 18px" }} />
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {data.domain_stats.map((s) => (
                <div key={s.lbl} style={{ background: "var(--bg3)", borderRadius: 10, padding: "12px 14px" }}>
                  <div style={{ fontFamily: "var(--font-syne)", fontWeight: 800, fontSize: 22, color: "var(--txt)", letterSpacing: "-0.5px" }}>{s.val}</div>
                  <div style={{ fontFamily: "var(--font-dm-sans)", fontSize: 11, color: "var(--txt-muted)", marginTop: 2 }}>{s.lbl}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 16, padding: "32px 28px" }}>
          <div style={{ marginBottom: 28 }}>
            <h3 style={{ fontFamily: "var(--font-syne)", fontWeight: 800, fontSize: 22, letterSpacing: "-0.7px", color: "var(--txt)", marginBottom: 6 }}>
          ...
          <div className="demo-recs-grid">
            {data.recommendations.map((r, i) => <RecCard key={i} rec={r} delay={i * 100} />)}
          </div>
        </div>
          ...
          {isDemo ? (
            ...
            <div style={{ textAlign: "center" }}>
              <p style={{ fontFamily: "var(--font-syne)", fontWeight: 700, fontSize: 18, color: "var(--txt)", marginBottom: 16, letterSpacing: "-0.5px" }}>
                ¿Quieres ver el reporte de <span style={{ color: "var(--accent)" }}>tu sitio</span>?
              </p>
              <div style={{
                ...
                <span style={{ display: "flex", alignItems: "center", padding: "0 12px", background: "var(--bg3)", borderRight: "1px solid var(--border)", fontFamily: "monospace", fontSize: 13, color: "var(--txt-faint)", whiteSpace: "nowrap" }}>
                  https://
                </span>
                <input
                  type="text"
                  ...
                  onKeyDown={(e) => e.key === "Enter" && url.trim() && onScan()}
                  placeholder="tunegocio.com"
                  style={{
                    flex: 1, background: "transparent", border: "none", outline: "none",
                    padding: "14px 16px", fontSize: 15, color: "var(--txt)",
                ...
                <button
                  ...
                  style={{ fontFamily: "var(--font-dm-sans)", borderRadius: 0, whiteSpace: "nowrap" }}
                >
                  Auditar →
                </button>
              </div>
            ...
            <div style={{ textAlign: "center" }}>
              <p style={{ fontFamily: "var(--font-syne)", fontWeight: 700, fontSize: 18, color: "var(--txt)", marginBottom: 16, letterSpacing: "-0.5px" }}>
                ...
                <span style={{ color: "var(--accent)" }}>accede al dashboard completo</span>
              </p>
              <button
                className="demo-btn-primary demo-pulse"
