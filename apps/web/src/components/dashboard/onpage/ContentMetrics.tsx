...
export function ContentMetrics({ content }: { content: OnPageData["content"] }) {
  ...
  // ARI: lower = easier. 1-5 = grade school, 6-8 = plain English, 9-12 = high school, 13+ = college
  const readLabel = ari == null ? "—"
    : ari <= 5  ? "Muy fácil"
    : ari <= 8  ? "Plain English"
    : ari <= 12 ? "Estándar"
    : "Difícil";

  const readColor = ari == null ? C.text4
    : ari <= 8  ? G
    : ari <= 12 ? AM
    : RD;

  // Normalize ARI to 0-100 bar (ARI 1-18 range, lower is better)
  const ariPct = ari == null ? null : Math.max(0, Math.min(100, 100 - ((ari - 1) / 17) * 100));

  const isLowWordCount = wc < 300;
    ...
    <Card style={{ padding: 24, height: "100%" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20 }}>
        <div style={{
          ...
          background: isLowWordCount ? "rgba(239,68,68,0.15)" : Gs,
          border: `1px solid ${isLowWordCount ? "rgba(239,68,68,0.35)" : Gb}`,
          color: isLowWordCount ? RD : G,
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{
            ...
            fontSize: 28, color: isLowWordCount ? RD : C.text, lineHeight: 1.1,
          }}>{wc}</div>
        </div>
        {isLowWordCount && (
          <Badge tone="red"><IconWarn size={11} /> Bajo</Badge>
        )}
      </div>
        ...
        <div style={{ marginBottom: 18 }}>
          <div style={{
            ...
            <span style={{
              ...
              color: readColor, fontWeight: 600,
            }}>{ari.toFixed(1)}</span>
          </div>
          <div style={{
